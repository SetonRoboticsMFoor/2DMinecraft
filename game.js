...
document.querySelector('#inventory').addEventListener('dblclick', event => { const item = event.target.closest('.item'); const tool = item?.querySelector('.crafted-icon.pickaxe, .crafted-icon.axe, .crafted-icon.shovel')?.classList[1]; if (tool && inventory[tool] > 0) { player.equippedTool = player.equippedTool === tool ? null : tool; updateInventory(); showToast(player.equippedTool ? `WOODEN ${tool.toUpperCase()} EQUIPPED` : 'TOOL UNEQUIPPED'); } });
